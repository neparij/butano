...
namespace
{
    [[nodiscard]] int _create_impl(const bg_palette_item& palette_item, bool required)
    {
        ...
        {
            id = bg_palettes_bank.find_bpp_8(colors, hash);

            if(id < 0)
            {
                id = bg_palettes_bank.create_bpp_8(colors, hash, palette_item.compression(), required);
            }
        }
    ...
    [[nodiscard]] int _create_new_impl(const bg_palette_item& palette_item, bool required)
    {
        ...
        {
            id = bg_palettes_bank.create_bpp_8(colors, palettes_bank::colors_hash(colors), palette_item.compression(), required);
        }
...
optional<bg_palette_ptr> bg_palette_ptr::find(const bg_palette_item& palette_item)
{
    ...
    if(palette_item.compression() == compression_type::NONE || ! bpp_4)
    {
        ...
        {
            id = bg_palettes_bank.find_bpp_8(colors, palettes_bank::colors_hash(colors));
        }
